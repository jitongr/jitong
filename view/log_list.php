...
<?php while ($value = $DB->fetch_array($query)) { ?>
	...
	<p class="date"><?=date('Y-m-d H:i:s',$value['edittime'])?> [<?php echo getcptype($value['sort']); ?>]<?php echo $value['id']; ?>  
 <?php echo $value['birth'].'-'.$value['die']; ?><?php echo $value['age']; ?>
	</p> <? if($value['img']){ ?> <img src="<?=$value['img']?>"> <br><? }?>
	<?php echo $value['content'] ?>
	<p class="count">
	<?=$value['otime']?>关联(<?=$value['f3']?>)
	浏览(<?=$value['words']?> )<a href="jt.php?id=<?=$value['id']?>">预览</a>
 <?php if(ISLOGIN ): ?><a href="jt.php?cp=<?=$value['id']?>">编辑</a><?php endif;?>
	</p>
